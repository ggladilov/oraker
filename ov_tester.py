...
def draw_results(results:Dict, source_image:np.ndarray, label_map:Dict):
    ...
    for idx, (*xyxy, conf, lbl) in enumerate(boxes):
        ...
        source_image = plot_one_box(xyxy, source_image, mask=mask, label=label, line_thickness=1)
    return source_image
...
def postprocess(
    ...
    for i, pred in enumerate(preds):
        ...
        if proto is None:
            ...
            continue
        if retina_mask:
            print(input_hw[0], shape[0], input_hw[0] / shape[0])
            pred[:, :4] = ops.scale_boxes(input_hw, pred[:, :4], shape).round()
...
Image.fromarray(det_model("./network_input.png")[0].plot()[:, :, ::-1])
...
input_image = np.array(Image.open("./network_input.png"))
...
# with open('./ov_tester_raw.txt', 'wt') as f:
#     numpy.savetxt(f, detections)
